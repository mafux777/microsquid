...
function buildResolvers(model) {
    let Query = {};
    let resolvers = { Query, ...(0, scalars_1.getScalarResolvers)() };
        ...
        switch (item.kind) {
            case 'entity':
                Query[(0, util_2.toQueryListField)(name)] = async (source, args, context, info) => {
                    ...
                    return new queryBuilder_1.QueryBuilder(model, db).executeSelect(name, args, fields);
                };
                ...
                Query[`${(0, util_1.toCamelCase)(name)}ById`] = async (source, args, context, info) => {
                    let fields = (0, requestedFields_1.requestedFields)(model, name, info);
                    let db = await context.openReaderTransaction.get();
                    let result = await new queryBuilder_1.QueryBuilder(model, db)
            ...
            case 'fts':
                Query[name] = async (source, args, context, info) => {
                    ...
                    let db = await context.openReaderTransaction.get();
                    return new queryBuilder_1.QueryBuilder(model, db).executeFulltextSearch(name, args, fields);
                };
...
async function resolveEntityConnection(model, entityName, args, info, db) {
    let response = {};
    ...
    if (fields.edges?.node) {
        let nodes = await new queryBuilder_1.QueryBuilder(model, db).executeSelect(entityName, listArgs, fields.edges.node);
        ...
        let listLength = await new queryBuilder_1.QueryBuilder(model, db).executeListCount(entityName, listArgs);
    ...
    if (fields.totalCount && response.totalCount == null) {
        response.totalCount = await new queryBuilder_1.QueryBuilder(model, db).executeSelectCount(entityName, listArgs.where);
    }
    return response;
}
