...
function generateOpenCrudQueries(model, dialect) {
    ...
    for (let name in model) {
        ...
        switch (item.kind) {
            ...
            case 'enum':
                ...
                break;
            case 'fts':
                (0, assert_1.default)(dialect == 'postgres', `Full-text search queries are not supported by ${dialect}`);
...
function buildServerSchema(model, dialect) {
    let scalars = ['ID'].concat(Object.keys(scalars_1.customScalars)).map(name => 'scalar ' + name).join('\n');
