...
exports.generateFtsMigrations = void 0;
...
function generateMigration(name, query, dir) {
    ...
    out.block(`module.exports = class ${name}${TS}`, () => {
        ...
        const sources = query.sources.map((src) => {
            const table = (0, util_1.toSnakeCase)(src.entity);
            const ginIndex = `${(0, util_1.toSnakeCase)(name)}_${(0, util_1.toSnakeCase)(src.entity)}_idx`;
            const tsvColumn = `${(0, util_1.toSnakeCase)(name)}_tsv`;
            const tsvectorValue = src.fields
                .map((f) => {
                return `setweight(to_tsvector('english', coalesce(${(0, util_1.toSnakeCase)(f)}, '')), 'A')`;
            })
                .join(' || ');
